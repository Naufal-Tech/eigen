...
const UserController = {
  ...
  infoUser: async function (req, res) {
    try {
      ...
      // Format timestamps for each presensi and user
      const formattedBook = user.borrowed_book.map((item) => {
  ...
  detail: async function (req, res) {
    ...
    try {
      ...
      const BookFormatted = user.borrowed_book.map((item) => {
        const bookCreatedAt = moment
          .tz(item.created_at, "Asia/Jakarta")
          .format("DD-MM-YYYY HH:mm:ss");
        const bookUpdatedAt = moment
      ...
      return response.ok(
        userWithBook,
        res,
        "Successfully retrieved user with book"
      );
    } catch (err) {
      return response.error(400, err.message, res, err);
    }
  ...
  get: async function (req, res) {
    try {
      ...
      if (codeMember) {
        queryObject.codeMember = { $regex: new RegExp(codeMember, "i") };
      }
      ...
      const usersWithFormattedBooks = users.map((user) => {
        const formattedBooks = user.borrowed_book.map((item) => {
          // Convert timestamps to readable format
          const formattedCreatedAt = moment(item.created_at).format(
            "DD-MM-YYYY HH:mm:ss"
          );
        ...
        return {
          ...user.toObject(),
          borrowed_book: formattedBooks,
          total_borrowed_books: user.borrowed_book.length,
          created_at: formattedCreatedAt,
          updated_at: formattedUpdatedAt,
        };
      });
